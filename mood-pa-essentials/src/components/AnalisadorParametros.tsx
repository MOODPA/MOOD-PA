...
import { simularExtracaoParametros, analisarParametrosUrbanisticos, CategoriaParametros } from '@/utils/calculoParametrosUrbanisticos';
import AnaliseParametros from '@/components/AnaliseParametros';
import { Upload, FileText, AlertCircle, Info } from 'lucide-react';

interface AnalisadorParametrosProps {
  onAnaliseCompleta?: (resultado: any) => void;
}

const AnalisadorParametros: FC<AnalisadorParametrosProps> = ({ onAnaliseCompleta }) => {
  const [files, setFiles] = useState<File[]>([]);
  const [tipoProjeto, setTipoProjeto] = useState<string>('residencial-unifamiliar');
  const [analisando, setAnalisando] = useState(false);
  const [resultadoAnalise, setResultadoAnalise] = useState<any>(null);
  const [categoriasParametros, setCategoriasParametros] = useState<CategoriaParametros[]>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Simular extração de parâmetros dos arquivos
      const parametros = await simularExtracaoParametros(files, tipoProjeto);
      setCategoriasParametros(parametros);
      
      // Analisar os parâmetros
      const resultado = analisarParametrosUrbanisticos(parametros);
  ...
  const handleParametroChange = (categoriaId: string, parametroId: string, valor: number) => {
    // Atualizar o valor do parâmetro
    const novasCategorias = categoriasParametros.map(categoria => {
      if (categoria.id === categoriaId) {
        const novosParametros = categoria.parametros.map(parametro => {
          if (parametro.id === parametroId) {
            return { ...parametro, valorProjeto: valor };
          }
          return parametro;
        });
        
        return { ...categoria, parametros: novosParametros };
      }
      return categoria;
    });
    
    setCategoriasParametros(novasCategorias);
    
    // Reanalisar os parâmetros
    const resultado = analisarParametrosUrbanisticos(novasCategorias);
    setResultadoAnalise(resultado);
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
      <h2 className="text-xl font-semibold mb-4">Análise de Parâmetros Urbanísticos</h2>
      ...
      {!resultadoAnalise && (
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end">
            ...
            >
              {analisando ? 'Analisando...' : 'Analisar Parâmetros Urbanísticos'}
            </button>
          </div>
        </form>
      )}
      ...
      {analisando && (
        <div className="text-center py-8">
          ...
          <p className="mt-4 text-gray-600 dark:text-gray-400">
            Analisando parâmetros urbanísticos dos arquivos...
          </p>
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-md flex items-start">
            ...
            <div className="text-sm text-blue-700 dark:text-blue-300">
              <p>
                A análise automática extraiu os parâmetros urbanísticos dos arquivos enviados.
                Se algum valor não foi detectado corretamente, você pode editá-lo manualmente clicando no ícone de edição.
              </p>
            </div>
          </div>
          
          <AnaliseParametros
            categoriasParametros={resultadoAnalise.categoriasAnalisadas}
            conformidade={resultadoAnalise.conformidade}
            parametrosNaoConformes={resultadoAnalise.parametrosNaoConformes}
...
export default AnalisadorParametros;
